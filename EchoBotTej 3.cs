...
{
    public class EchoBot3 : ActivityHandler
    {
        ...
        private static async Task<List<Leave>> GetDynamicQueryForSP(UAT_LUIS_Entity UAT_LUIS)//(string QueryIntent, string QueryEntity)
        {
            ...
            if (entityList.Contains("date") == true)
            {
                ...
                if (indexVal.Count == 1)
                {
                    ...
                    string InputDate = Convert.ToDateTime(UAT_LUIS.entities[indexVal[0]].entity.ToString().Replace(" ", string.Empty)).ToString(SystemDateFormate);
                    //system date formate output  
                    DateTime ResultedDate = DateTime.Parse(InputDate);


                    leaveStartDate = leaveEndDate  = ResultedDate.ToString("MM-DD-YYYY");
                ...
                else
                {
                    for (int i = 0; i < indexVal.Count; i++)
                    {
                        if (UAT_LUIS.entities[indexVal[i]].role.ToString().Equals("FromDate"))
                        {
                            ...
                            String str = Convert.ToDateTime(UAT_LUIS.entities[indexVal[i]].entity.ToString().Replace(" ", string.Empty)).ToString("yyyy-dd-mmTHH:mm:ssZ");
                            ...
                            string InputDate = Convert.ToDateTime(str).ToString(SystemDateFormate);
                            //system date formate output  
                            DateTime ResultedDate = DateTime.Parse(InputDate);


                            // leaveStartDate = ResultedDate.ToString("MM-DD-YYYY");

                          //  leaveStartDate = Convert.ToDateTime(ResultedDate.ToString()).ToString();
                            leaveStartDate = Convert.ToDateTime(str).ToString("MM/dd/yyyy");
                            String d = "";

                            //Convert.ToDateTime(ty.ToString().Replace(" ", string.Empty)).ToString("dd-MM-YYYY,THH:mm:ssZ");
                            ...
                            //dateString = UAT_LUIS.entities[indexVal[i]].entity.ToString();
                            //format = "mm/dd/yyyy";
                            ...
                            //String origionalDate = Convert.ToDateTime(UAT_LUIS.entities[indexVal[i]].entity.ToString()).ToString("MM/dd/yyyy");//"12/20/2013"; // Format : MM/dd/yyyy
                            //string origionalFormat = CultureInfo.CurrentCulture.DateTimeFormat.ShortDatePattern;
...

                        {
                            ...
                            leaveEndDate = ResultedDate.ToString("MM-DD-YYYY");
                ...
                case "last days":
                    {
                        ...
                        {
                            //   case "last days":
                            leaveStartDate = DateTime.UtcNow.AddDays(-6).ToString("yyyy-MM-ddTHH:mm:ssZ");
                            leaveEndDate = DateTime.UtcNow.AddDays(0).ToString("yyyy-MM-ddTHH:mm:ssZ");
                            labelToMakeQuery = "dateRange";
                        }
                        else
